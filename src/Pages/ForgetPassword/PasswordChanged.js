import React from 'react';
import PasswordSuccessLogo from '../../utils/Account/Icons/PasswordSuccessLogo.svg';
import { useNavigate } from 'react-router-dom';
import SubHeader from '../../Layout/SubHeader';

const PasswordChanged = () => {
  const navigate = useNavigate();
  return (
    <>
    <SubHeader></SubHeader>
    <div className="py-[90px] px-[75px] flex flex-col justify-center">
      <div className="flex items-center justify-center gap-2 mb-[50px]">
        <img src={PasswordSuccessLogo} alt="PasswordSuccessLogo" className="cursor-pointer h-[64px] w-[64px]" />
      </div>
      <div className="text-[22px] flex justify-center mb-[8px] font-[700] text-[#19224C]">Password Changed!</div>
      <div className="flex justify-center px-[10px] text-center text-[#6D77A0] mb-[20px] flex-col text-[12px] gap-2">
        Your password has been changed successfully.
      </div>
      <button
        className="w-full focus:outline-none rounded-md bg-[#1678F2] px-1 py-[16px] text-[16px] font-[700] text-white hover:opacity-90 disabled:cursor-none disabled:opacity-50"
        onClick={() => navigate('/login')}
      >
        Continue
      </button>
    </div>
    </>
  );
};
export default PasswordChanged;
